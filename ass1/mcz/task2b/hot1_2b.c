...
int printRoundStartValues(my_rank, round, myNumbers, mValue);
int world_size;
int mValue;
int commLine = 9999;
...
int main(int argc, char *argv[])
{
	...
	{
		printRoundStartValues(my_rank, round, myNumbers, mValue);
		...
		if (round % 2 == 0)
		{
			...
			if (comSucc == 1)
			{
				memcpy(total, myNumbers, mValue * sizeof(int));				// copy n floats from x to total[0]...total[n]
				memcpy(total + mValue, neighNumbers, mValue * sizeof(int)); // copy n floats from y to total[n+1]...total[2n]

				qsort(total, 2 * mValue, sizeof(int), compare);

				if (my_rank % 2 == 0)
				{
					memcpy(myNumbers, total, mValue * sizeof(int));
				}
				else
				{
					memcpy(myNumbers, total + mValue, mValue * sizeof(int));
				}
			}
		}
		else
		{
			...
			{
				memcpy(total, myNumbers, mValue * sizeof(int));				// copy n floats from x to total[0]...total[n]
				memcpy(total + mValue, neighNumbers, mValue * sizeof(int)); // // copy n floats from x to total[n+1]...total[2n]
...
int printRoundStartValues(int my_rank, int round, int myNumbers[], int mValue)
{
	for (int i = 0; i < world_size; i++)
	{
		MPI_Barrier(MPI_COMM_WORLD);
		if (my_rank == i)
		{
			printf("(%d|%d| ", round, my_rank);
			printArray(myNumbers, mValue);
			printf(")\n");
		}
		else
		{
